...
if(isset($_POST["action"]) && $_POST["action"] == "contact"){
    ...
    $nameRegexp = "/^([A-ZĐŠĆŽČ][a-zšđćžč]{0,14})+(\s[A-ZĐŠĆŽČ][a-zšđćžč]{0,14})*$/";
    ...
    if(count($greske) == 0){
        ...
        $pripremaUnos = $db -> prepare("INSERT INTO contact_forms VALUES (NULL, :message, :email, :name, :date)");
        $pripremaUnos -> bindParam(":message", $message);
        $pripremaUnos -> bindParam(":email", $email);
        $pripremaUnos -> bindParam(":name", $name);
        $date = date("Y-m-d H:i:s", time());
        $pripremaUnos -> bindParam(":date", $date);
        try{
